...
| 样式 | Tailwind CSS + CSS variables | 能快速实现设计 token、响应式与状态样式 | 大型 UI 套件，容易形成模板感 |
| 无障碍基础 | Radix primitives 或原生语义组件 | 处理焦点、键盘与弹层语义 | 自造所有交互基础组件 |
| 数据请求 | TanStack Query | 缓存、重试、加载和错误状态清晰 | 把请求状态散落在组件中 |
| Agent API | FastAPI + Pydantic | 结构化 schema、自动文档、异步接口和测试友好 | 把 Agent 逻辑直接写在前端 |
| Workflow | LangGraph 或等价显式图编排 | 支持状态、条件路由、检查点和人工中断 | 完全自由的 ReAct 循环 |
| 硬规则 | Python 纯函数 + 版本化规则配置 | 金额和权限可单测、可审计 | 让 LLM 直接计算退款金额 |
| 数据 | SQLite + SQLModel/SQLAlchemy | 本地可运行、支持状态持久化和事务 | 纯前端写死 JSON 结果 |
| 政策检索 | 结构化过滤优先，轻量全文检索补充 | 只有 8 至 12 条模拟政策，不需要向量数据库 | 为展示 RAG 而引入复杂向量设施 |
| LLM | 支持 tool calling 与结构化输出的模型，通过适配层接入 | 供应商可替换，便于无 Key 降级 | 业务代码与单一模型 SDK 深度耦合 |
